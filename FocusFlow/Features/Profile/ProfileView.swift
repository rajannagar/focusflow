...
private struct BadgeSystem {
    static func allBadges(totalMinutes: Int, totalSessions: Int, bestStreak: Int, tasksCompleted: Int, goalsHit: Int, longestSession: Int, morningCount: Int, nightCount: Int) -> [Badge] {
        [
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack(alignment: .leading) {
                ...
                RoundedRectangle(cornerRadius: 4).fill(LinearGradient(colors: [color, color.opacity(0.7)], startPoint: .leading, endPoint: .trailing)).frame(width: geo.size.width * progress)
            }
        }.frame(height: 8)
    }
}
...
private struct LevelBadge: View {
    ...
    var body: some View {
        ZStack {
            Circle().fill(LinearGradient(colors: [color, color.opacity(0.6)], startPoint: .topLeading, endPoint: .bottomTrailing)).frame(width: size, height: size)
            Circle().stroke(Color.white.opacity(0.3), lineWidth: 2).frame(width: size, height: size)
            Text("\(level)").font(.system(size: size * 0.45, weight: .bold, design: .rounded)).foregroundColor(.white)
...
private struct BadgeCard: View {
    ...
    var body: some View {
        Button(action: onTap) {
            VStack(spacing: 8) {
                ZStack {
                    Circle().fill(badge.isUnlocked ? badge.color.opacity(0.2) : Color.white.opacity(0.05)).frame(width: size, height: size)
                    if !badge.isUnlocked {
                        Circle().trim(from: 0, to: badge.progress).stroke(badge.color.opacity(0.4), style: StrokeStyle(lineWidth: 3, lineCap: .round)).frame(width: size - 4, height: size - 4).rotationEffect(.degrees(-90))
                    }
                    Image(systemName: badge.icon).font(.system(size: size * 0.4, weight: .semibold)).foregroundColor(badge.isUnlocked ? badge.color : .white.opacity(0.25))
                    if badge.isUnlocked { Circle().fill(badge.color.opacity(0.3)).frame(width: size + 10, height: size + 10).blur(radius: 10) }
                }
                Text(badge.name).font(.system(size: 10, weight: .semibold)).foregroundColor(badge.isUnlocked ? .white : .white.opacity(0.4)).lineLimit(1)
            }.frame(width: size + 10)
        }.buttonStyle(.plain)
    }
}
...
private struct WeekDayDot: View {
    ...
    var body: some View {
        VStack(spacing: 6) {
            Text(day).font(.system(size: 10, weight: .medium)).foregroundColor(.white.opacity(0.4))
            Circle().fill(isActive ? color : Color.white.opacity(0.1)).frame(width: 28, height: 28)
...
private struct RingProgress: View {
    ...
    var body: some View {
        ZStack {
            ...
            Circle().trim(from: 0, to: min(1.0, progress)).stroke(color, style: StrokeStyle(lineWidth: lineWidth, lineCap: .round)).rotationEffect(.degrees(-90))
        }
    }
}
...
struct ProfileView: View {
    ...
    private var weekDays: [(day: String, date: Date, isActive: Bool, isToday: Bool)] {
        ...
        var result: [(String, Date, Bool, Bool)] = []; var cursor = interval.start; var dayIndex = cal.component(.weekday, from: cursor) - 1
        for _ in 0..<7 {
            result.append((dayLetters[dayIndex % 7], cursor, activeDaysThisWeek.contains(cursor), cal.isDateInToday(cursor)))
            dayIndex += 1; guard let next = cal.date(byAdding: .day, value: 1, to: cursor) else { break }; cursor = next
        }
        return result
    }
    ...
    private var identityCard: some View {
        VStack(spacing: 0) {
            HStack {
                ...
                Spacer()
                Button { Haptics.impact(.light); showingSettings = true } label: {
                    Image(systemName: "gearshape.fill").font(.system(size: 16)).foregroundColor(.white.opacity(0.4)).frame(width: 36, height: 36).background(Color.white.opacity(0.06)).clipShape(Circle())
                }
            }.padding(.horizontal, 16).padding(.top, 12)
            
            Button { Haptics.impact(.light); showingEditProfile = true } label: {
                ZStack {
                    RingProgress(progress: levelProgress, lineWidth: 4, color: theme.accentPrimary).frame(width: 96, height: 96)
                    profileAvatar(size: 80)
                    VStack { Spacer(); HStack { Spacer(); LevelBadge(level: currentLevel, color: theme.accentPrimary, size: 32).offset(x: 6, y: 6) } }.frame(width: 96, height: 96)
                }
            }.buttonStyle(.plain)
            
            Text(displayName).font(.system(size: 22, weight: .bold)).foregroundColor(.white).padding(.top, 12)
            
            HStack(spacing: 6) {
                Text(currentTitle).font(.system(size: 14, weight: .bold, design: .rounded)).foregroundColor(theme.accentPrimary)
                Button { Haptics.impact(.light); showingLevelInfo = true } label: {
                    Image(systemName: "info.circle").font(.system(size: 14)).foregroundColor(.white.opacity(0.4))
                }
            }.padding(.horizontal, 14).padding(.vertical, 5).background(theme.accentPrimary.opacity(0.15)).clipShape(Capsule()).padding(.top, 6)
            
            VStack(spacing: 6) {
                XPProgressBar(progress: levelProgress, color: theme.accentPrimary)
                HStack {
                    Text("\(totalXP) XP").font(.system(size: 11, weight: .semibold)).foregroundColor(.white.opacity(0.5))
                    Spacer()
                    Text(currentLevel < 50 ? "\(xpToNext) XP to Level \(currentLevel + 1)" : "MAX LEVEL").font(.system(size: 11, weight: .semibold)).foregroundColor(currentLevel < 50 ? .white.opacity(0.4) : theme.accentPrimary)
                }
            }.padding(.horizontal, 24).padding(.top, 16).padding(.bottom, 20)
        }.background(Color.white.opacity(0.04)).clipShape(RoundedRectangle(cornerRadius: 24, style: .continuous)).overlay(RoundedRectangle(cornerRadius: 24, style: .continuous).stroke(Color.white.opacity(0.06), lineWidth: 1))
    }
    ...
    private var badgesSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            HStack {
                Text("BADGES").font(.system(size: 11, weight: .bold, design: .rounded)).foregroundColor(.white.opacity(0.4)).tracking(1.5)
                Spacer()
                Button { Haptics.impact(.light); showingAllBadges = true } label: {
                    HStack(spacing: 4) {
                        Text("\(unlockedBadges.count)/\(allBadges.count)").font(.system(size: 12, weight: .bold)).foregroundColor(theme.accentPrimary)
                        Image(systemName: "chevron.right").font(.system(size: 10, weight: .bold)).foregroundColor(theme.accentPrimary.opacity(0.6))
                    }
                }
            }
            .padding(.bottom, 4)
            ...
            if unlockedBadges.isEmpty && inProgressBadges.isEmpty {
                VStack(spacing: 8) {
                    Image(systemName: "trophy").font(.system(size: 28)).foregroundColor(.white.opacity(0.2))
                    Text("Start focusing to earn badges!").font(.system(size: 13, weight: .medium)).foregroundColor(.white.opacity(0.4))
                }.frame(maxWidth: .infinity).padding(.vertical, 24)
            } else {
                ScrollView(.horizontal, showsIndicators: false) {
                    HStack(spacing: 16) {
                        ForEach(unlockedBadges.prefix(4)) { badge in BadgeCard(badge: badge, size: 52) { Haptics.impact(.light); selectedBadge = badge } }
                        if !inProgressBadges.isEmpty {
                            Rectangle().fill(Color.white.opacity(0.1)).frame(width: 1, height: 50)
                            ForEach(inProgressBadges.prefix(2)) { badge in BadgeCard(badge: badge, size: 52) { Haptics.impact(.light); selectedBadge = badge } }
                        }
                    }
                    .padding(.horizontal, 4)
                    .padding(.top, 10) // Top padding for glow
                    .padding(.bottom, 12) // Bottom padding for glow/shadow
                }
                .padding(.top, -6) // Offset to keep visual alignment
            }
        }
        ...
        VStack(alignment: .leading, spacing: 14) {
            ...
            HStack(spacing: 16) {
                VStack(alignment: .leading, spacing: 4) {
                    Text("Today").font(.system(size: 12, weight: .semibold)).foregroundColor(.white.opacity(0.5))
                    HStack(spacing: 12) {
                        Label("\(todayMinutes)m", systemImage: "clock")
                        if tasksTotalToday > 0 { Label("\(tasksCompletedToday)/\(tasksTotalToday)", systemImage: "checklist") }
            ...
            }.padding(12).background(Color.white.opacity(0.04)).clipShape(RoundedRectangle(cornerRadius: 12, style: .continuous))
        }.padding(16).background(Color.white.opacity(0.04)).clipShape(RoundedRectangle(cornerRadius: 20, style: .continuous))
    }
    
    private func statCell(icon: String, value: String, label: String, color: Color) -> some View {
        HStack(spacing: 12) {
            Image(systemName: icon).font(.system(size: 18)).foregroundColor(color).frame(width: 36, height: 36).background(color.opacity(0.15)).clipShape(Circle())
    ...
    private var weekCard: some View {
        VStack(alignment: .leading, spacing: 12) {
            HStack {
                Text("THIS WEEK").font(.system(size: 11, weight: .bold, design: .rounded)).foregroundColor(.white.opacity(0.4)).tracking(1.5)
                Spacer()
                Text("\(thisWeekMinutes)m • \(activeDaysThisWeek.count)/7 days").font(.system(size: 12, weight: .semibold)).foregroundColor(.white.opacity(0.5))
            }
            HStack(spacing: 0) {
                ForEach(Array(weekDays.enumerated()), id: \.offset) { _, day in
                    WeekDayDot(day: day.day, isActive: day.isActive, isToday: day.isToday, color: theme.accentPrimary).frame(maxWidth: .infinity)
                }
            }
            if currentStreak > 0 {
                HStack(spacing: 6) {
                    Image(systemName: "flame.fill").foregroundColor(.orange)
                    Text("\(currentStreak) day streak").font(.system(size: 13, weight: .semibold)).foregroundColor(.white.opacity(0.7))
                }.padding(.top, 4)
            }
        }.padding(16).background(Color.white.opacity(0.04)).clipShape(RoundedRectangle(cornerRadius: 20, style: .continuous))
    }
    ...
    private var accountSection: some View {
        VStack(spacing: 0) {
            ...
            case .authenticated(let s):
                HStack {
                    VStack(alignment: .leading, spacing: 2) {
                        Text(s.isGuest ? "Guest" : (s.email ?? "Signed in")).font(.system(size: 14, weight: .semibold)).foregroundColor(.white.opacity(0.8))
                        HStack(spacing: 4) {
                            Circle().fill(s.isGuest ? Color.orange : Color.green).frame(width: 6, height: 6)
                            Text(s.isGuest ? "Local data" : "Synced").font(.system(size: 11, weight: .medium)).foregroundColor(.white.opacity(0.4))
                        }
                    }
                    Spacer()
                    Button { Haptics.impact(.medium); auth.signOut() } label: {
                        Text("Sign Out").font(.system(size: 12, weight: .semibold)).foregroundColor(.white.opacity(0.6)).padding(.horizontal, 12).padding(.vertical, 6).background(Color.white.opacity(0.08)).clipShape(Capsule())
                    }
                }.padding(14)
            default: EmptyView()
            }
        }.background(Color.white.opacity(0.04)).clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous))
    }
    
    // MARK: - Upgrade Card
    private var upgradeCard: some View {
        Button { Haptics.impact(.medium); showingPaywall = true } label: {
            HStack(spacing: 12) {
                Image(systemName: "crown.fill").font(.system(size: 24)).foregroundColor(.yellow)
                VStack(alignment: .leading, spacing: 2) {
                    Text("Upgrade to Pro").font(.system(size: 16, weight: .bold)).foregroundColor(.white)
                    Text("Unlock all features & 2X XP").font(.system(size: 12, weight: .medium)).foregroundColor(.white.opacity(0.6))
                }
                Spacer()
                Image(systemName: "chevron.right").font(.system(size: 14, weight: .semibold)).foregroundColor(.yellow.opacity(0.6))
            }.padding(16).background(LinearGradient(colors: [Color.yellow.opacity(0.15), Color.orange.opacity(0.1)], startPoint: .leading, endPoint: .trailing))
            .clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous)).overlay(RoundedRectangle(cornerRadius: 16, style: .continuous).stroke(Color.yellow.opacity(0.3), lineWidth: 1))
        }.buttonStyle(.plain)
    }
}
        ...
        ZStack {
            ...
            ScrollView {
                VStack(spacing: 24) {
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            Text("Levels & XP").font(.system(size: 22, weight: .bold)).foregroundColor(.white)
                            Text("How to level up").font(.system(size: 14, weight: .medium)).foregroundColor(.white.opacity(0.5))
                        }
                        Spacer()
                        Button { dismiss() } label: { Image(systemName: "xmark.circle.fill").font(.system(size: 26)).foregroundColor(.white.opacity(0.3)) }
                    }
                    VStack(spacing: 8) {
                        LevelBadge(level: currentLevel, color: theme.accentPrimary, size: 60)
                        Text("Level \(currentLevel)").font(.system(size: 18, weight: .bold)).foregroundColor(.white)
                        Text("\(totalXP) XP total").font(.system(size: 14, weight: .medium)).foregroundColor(.white.opacity(0.5))
                    }.padding(.vertical, 16)
                    VStack(alignment: .leading, spacing: 12) {
                        Text("EARN XP BY").font(.system(size: 11, weight: .bold, design: .rounded)).foregroundColor(.white.opacity(0.4)).tracking(1.5)
                    ...
                    }.padding(16).background(Color.white.opacity(0.06)).clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous))
                    VStack(alignment: .leading, spacing: 12) {
                        Text("LEVEL TITLES").font(.system(size: 11, weight: .bold, design: .rounded)).foregroundColor(.white.opacity(0.4)).tracking(1.5)
                        ForEach(levelTiers, id: \.title) { tier in
                            HStack {
                                Text("Lv \(tier.range)").font(.system(size: 13, weight: .semibold, design: .monospaced)).foregroundColor(.white.opacity(0.5)).frame(width: 60, alignment: .leading)
                                Text(tier.title).font(.system(size: 14, weight: .bold)).foregroundColor(tier.color)
                                Spacer()
                                if isCurrentTier(tier.range) {
                                    Text("CURRENT").font(.system(size: 9, weight: .bold)).foregroundColor(theme.accentPrimary).padding(.horizontal, 6).padding(.vertical, 2).background(theme.accentPrimary.opacity(0.2)).clipShape(Capsule())
                                }
                            }.padding(.vertical, 6)
                        }
                    }.padding(16).background(Color.white.opacity(0.06)).clipShape(RoundedRectangle(cornerRadius: 16, style: .continuous))
                ...
                VStack(alignment: .leading, spacing: 24) {
                    HStack {
                        VStack(alignment: .leading, spacing: 4) {
                            ...
                            Text("\(unlockedBadges.count) of \(badges.count) unlocked").font(.system(size: 14, weight: .medium)).foregroundColor(.white.opacity(0.5))
                        }
                        Spacer()
                        Button { dismiss() } label: { Image(systemName: "xmark.circle.fill").font(.system(size: 26)).foregroundColor(.white.opacity(0.3)) }
                    ...
                    if !unlockedBadges.isEmpty {
                        VStack(alignment: .leading, spacing: 12) {
                            Text("UNLOCKED").font(.system(size: 11, weight: .bold, design: .rounded)).foregroundColor(.white.opacity(0.4)).tracking(1.5).padding(.horizontal, 20)
                            LazyVGrid(columns: columns, spacing: 20) {
                                ForEach(unlockedBadges) { badge in BadgeCard(badge: badge, size: 56) { selectedBadge = badge } }
                            }.padding(.horizontal, 16)
                        }
                    }
                    if !lockedBadges.isEmpty {
                        VStack(alignment: .leading, spacing: 12) {
                            Text("IN PROGRESS").font(.system(size: 11, weight: .bold, design: .rounded)).foregroundColor(.white.opacity(0.4)).tracking(1.5).padding(.horizontal, 20)
                            LazyVGrid(columns: columns, spacing: 20) {
                                ForEach(lockedBadges) { badge in BadgeCard(badge: badge, size: 56) { selectedBadge = badge } }
                            }.padding(.horizontal, 16)
                        }
                    }
                }.padding(.bottom, 40)
...
private struct BadgeDetailSheet: View {
    ...
    var body: some View {
        ZStack {
            PremiumAppBackground(theme: theme, showParticles: false)
            VStack(spacing: 24) {
                HStack { Spacer(); Button { dismiss() } label: { Image(systemName: "xmark.circle.fill").font(.system(size: 26)).foregroundColor(.white.opacity(0.3)) } }.padding(.horizontal, 20).padding(.top, 16)
                Spacer()
                ZStack {
                    if badge.isUnlocked { Circle().fill(badge.color.opacity(0.2)).frame(width: 140, height: 140).blur(radius: 30) }
                    Circle().fill(badge.isUnlocked ? badge.color.opacity(0.2) : Color.white.opacity(0.05)).frame(width: 100, height: 100)
                    if !badge.isUnlocked { Circle().trim(from: 0, to: badge.progress).stroke(badge.color.opacity(0.5), style: StrokeStyle(lineWidth: 4, lineCap: .round)).frame(width: 96, height: 96).rotationEffect(.degrees(-90)) }
                    Image(systemName: badge.icon).font(.system(size: 44, weight: .semibold)).foregroundColor(badge.isUnlocked ? badge.color : .white.opacity(0.25))
                }
                VStack(spacing: 8) {
                    Text(badge.name).font(.system(size: 24, weight: .bold)).foregroundColor(.white)
                    Text(badge.description).font(.system(size: 15, weight: .medium)).foregroundColor(.white.opacity(0.6))
                    if badge.isUnlocked {
                        Text("UNLOCKED").font(.system(size: 12, weight: .bold, design: .rounded)).foregroundColor(badge.color).padding(.top, 8)
                    } else {
                        Text("\(badge.currentValue)/\(badge.targetValue)").font(.system(size: 18, weight: .bold, design: .rounded)).foregroundColor(.white.opacity(0.7)).padding(.top, 8)
                        GeometryReader { geo in
                            ZStack(alignment: .leading) {
                                RoundedRectangle(cornerRadius: 4).fill(Color.white.opacity(0.1))
                                RoundedRectangle(cornerRadius: 4).fill(badge.color).frame(width: geo.size.width * badge.progress)
                            }
                        }.frame(height: 6).frame(maxWidth: 200).padding(.top, 4)
                    }
                }
                VStack(spacing: 8) {
                    Text("HOW TO ACHIEVE").font(.system(size: 11, weight: .bold, design: .rounded)).foregroundColor(.white.opacity(0.4)).tracking(1.5)
                    Text(badge.howToAchieve).font(.system(size: 14, weight: .medium)).foregroundColor(.white.opacity(0.7)).multilineTextAlignment(.center).padding(.horizontal, 32)
                }.padding(.top, 16)
                Spacer(); Spacer()
            }
...
// MARK: - Settings Sheet (Updated with notifications, subscription management, type-to-reset)
private struct SettingsSheet: View {
    ...
    var body: some View {
        ...
        .alert("Reset All Data", isPresented: $showingReset) {
            ...
            Text("This will permanently delete all your focus sessions, stats, and progress. Type RESET to confirm.")
        }
    ...
    private var themeSection: some View {
        SettingsSectionView(title: "THEME") {
            ScrollView(.horizontal, showsIndicators: false) {
                ...
                .padding(.horizontal, 4)
                .padding(.vertical, 8) // Extra padding so selection ring doesn't get clipped
            }
        }
    }
        ...
        NavigationStack {
            ZStack {
                ...
                VStack(spacing: 24) {
                    ZStack {
                        if let data = settings.profileImageData, let ui = UIImage(data: data) {
                            Image(uiImage: ui).resizable().scaledToFill().frame(width: 80, height: 80).clipShape(Circle())
                        } else {
                            let opt = AvatarLibrary.option(for: settings.avatarID)
                            Circle().fill(LinearGradient(colors: [opt.gradientA, opt.gradientB], startPoint: .topLeading, endPoint: .bottomTrailing)).frame(width: 80, height: 80)
                                .overlay(Image(systemName: opt.symbol).font(.system(size: 32, weight: .semibold)).foregroundColor(.white.opacity(0.9)))
                        }
                    }
                    HStack(spacing: 12) {
                        PhotosPicker(selection: $selectedPhoto, matching: .images) {
                            Label("Photo", systemImage: "photo").font(.system(size: 13, weight: .semibold)).foregroundColor(.white).padding(.horizontal, 14).padding(.vertical, 8).background(Color.white.opacity(0.1)).clipShape(Capsule())
                        }
                        Button { showingAvatars = true } label: {
                            Label("Avatar", systemImage: "face.smiling").font(.system(size: 13, weight: .semibold)).foregroundColor(.white).padding(.horizontal, 14).padding(.vertical, 8).background(Color.white.opacity(0.1)).clipShape(Capsule())
                        }
                        if settings.profileImageData != nil {
                            Button { settings.profileImageData = nil } label: {
                                Image(systemName: "xmark").font(.system(size: 11, weight: .bold)).foregroundColor(.white.opacity(0.5)).frame(width: 28, height: 28).background(Color.white.opacity(0.1)).clipShape(Circle())
                            }
                        }
                    }
                    VStack(alignment: .leading, spacing: 6) {
                        Text("Display Name").font(.system(size: 12, weight: .semibold)).foregroundColor(.white.opacity(0.5))
                        TextField("Your name", text: $name).font(.system(size: 15, weight: .medium)).foregroundColor(.white).padding(12).background(Color.white.opacity(0.08)).clipShape(RoundedRectangle(cornerRadius: 10)).tint(.white)
                    }.padding(.horizontal, 20)
                    Button { settings.displayName = name; dismiss() } label: {
                        Text("Save").font(.system(size: 15, weight: .bold)).foregroundColor(.black).frame(maxWidth: .infinity).padding(.vertical, 14).background(theme.accentPrimary).clipShape(RoundedRectangle(cornerRadius: 12))
                    }.padding(.horizontal, 20)
                    Spacer()
                }.padding(.top, 20)
            }.navigationTitle("Edit Profile").navigationBarTitleDisplayMode(.inline)
            .toolbar { ToolbarItem(placement: .navigationBarTrailing) { Button { dismiss() } label: { Image(systemName: "xmark.circle.fill").font(.system(size: 22)).foregroundColor(.white.opacity(0.3)) } } }
            .toolbarBackground(.hidden, for: .navigationBar)
        }.onAppear { name = settings.displayName }
        .sheet(isPresented: $showingAvatars) { AvatarPickerSheet(avatarID: $settings.avatarID, theme: theme) }
        .onChange(of: selectedPhoto) { _, item in guard let item else { return }; Task { if let data = try? await item.loadTransferable(type: Data.self) { await MainActor.run { settings.profileImageData = data } } } }
    }
}

// MARK: - Avatar Library
private struct AvatarOption: Identifiable, Equatable { let id: String, symbol: String, gradientA: Color, gradientB: Color }
...
private struct AvatarPickerSheet: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: 16) {
                HStack {
                    Text("Choose Avatar").font(.system(size: 18, weight: .bold)).foregroundColor(.white)
                    Spacer()
                    Button { dismiss() } label: { Image(systemName: "xmark.circle.fill").font(.system(size: 22)).foregroundColor(.white.opacity(0.3)) }
                }.padding(.horizontal, 20).padding(.top, 16)
                LazyVGrid(columns: columns, spacing: 16) {
                    ForEach(AvatarLibrary.options) { opt in
                        Button { avatarID = opt.id; dismiss() } label: {
                            Circle().fill(LinearGradient(colors: [opt.gradientA, opt.gradientB], startPoint: .topLeading, endPoint: .bottomTrailing)).frame(width: 60, height: 60)
                                .overlay(Image(systemName: opt.symbol).font(.system(size: 26, weight: .semibold)).foregroundColor(.white.opacity(0.9)))
...
#Preview { ProfileView().environmentObject(ProEntitlementManager()) }
