...
extension ProgressStore {
    ...
    func mergeRemoteSessions(_ remoteSessions: [ProgressSession]) {
        ...
        // This requires making sessions settable or adding an internal method
        applyRemoteSessionState(allSessions)
    }
    
    /// Internal method to apply remote state without triggering sync
    func applyRemoteSessionState(_ newSessions: [ProgressSession]) {
        // Note: This may need the sessions property to be internal(set)
        // or use a flag to prevent persist during remote apply
